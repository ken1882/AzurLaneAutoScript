...
class IslandPostManage(IslandInteract):
    ...
    def process_slot(self, slot: Button, target_product: str=''):
        ...
        timeout  = Timer(10.0).reset()
        idle_timer = Timer(2.0)
        while 1:
            if timeout.reached():
                logger.warning('Timeout waiting for slot processing, slot probably locked')
                return
